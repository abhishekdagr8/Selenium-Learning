...
public class ComplexReportFactory {
	...
	private synchronized static ExtentReports getExtentReport() {
		if (reporter == null) {
			...
			reporter = new ExtentReports(System.getProperty("user.dir") +"/test-output/Results"+GenericReusable.reportStartTime()+"/Report"+GenericReusable.currentTime()+".html", true, DisplayOrder.NEWEST_FIRST);
		}
		return reporter;
	}
	...
	public synchronized static void getResult(ITestResult result){
		ExtentTest test = getTest();
		 ...
		 //logger.log(LogStatus.FAIL, "Test Case Failed is "+result.getName());
			 try {
				 test.log(LogStatus.FAIL, "Test Case Failed<br /> "+result.getThrowable(),"SnapShot Below: "+ test.addScreenCapture(GenericReusable.getScreenShot(result.getName())));
		 }catch(Exception e) {
